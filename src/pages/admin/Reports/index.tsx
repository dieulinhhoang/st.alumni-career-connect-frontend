  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, color: '#94a3b8', fontWeight: 500, marginBottom: 4, letterSpacing: '0.01em' }}>
          {label}
        </div>
        <div style={{ fontSize: 26, fontWeight: 700, color: '#0f172a', lineHeight: 1.2, fontVariantNumeric: 'tabular-nums' }}>
          {value}
        </div>
        <div style={{ fontSize: 12, color: accentText, fontWeight: 500, marginTop: 4, background: accentLight, display: 'inline-block', padding: '1px 8px', borderRadius: 20 }}>
          {sub}
        </div>
      </div>
    </div>
...
function MiniBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span style={{ fontSize: 12, color: '#475569', fontVariantNumeric: 'tabular-nums', minWidth: 28, textAlign: 'right' }}>{value}</span>
    </div>
  )
}
...
function KpiRing({ label, value, color, desc }: { label: string; value: string; color: string; desc: string }) {
  ...
  return (
    <div style={{
      ...
      <svg width={96} height={96} viewBox="0 0 96 96" style={{ display: 'block', margin: '0 auto 12px' }}>
        ...
        />
        <text x={48} y={52} textAnchor="middle" fontSize={16} fontWeight={700} fill="#0f172a">{value}%</text>
      </svg>
...
export default function ReportsPage() {
  ...
  const maxGraduates = Math.max(...filteredMajorSummary.map(r => r.totalGraduates), 1)
  ...
  const overviewColumns: ColumnsType<MajorSummaryRow> = [
    {
      ...
      fixed: 'left',
      width: 220,
      render: (_: unknown, r: MajorSummaryRow) => (
        <div>
          <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 13 }}>{r.majorName}</div>
          <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>
            <Text code style={{ fontSize: 11, background: '#f8fafc', border: '1px solid #e2e8f0', padding: '0 4px', borderRadius: 4 }}>{r.majorCode}</Text>
          </div>
        </div>
      ),
    },
    { title: 'Khoa', dataIndex: 'facultyName', key: 'facultyName', width: 180, render: (v: string) => <span style={{ fontSize: 12, color: '#475569' }}>{v}</span> },
    {
      title: 'SV tốt nghiệp',
      key: 'graduates_bar',
      width: 160,
      render: (_: unknown, r: MajorSummaryRow) => (
        <div>
          ...
          <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Phản hồi: {r.responses} ({((r.responses/r.totalGraduates)*100).toFixed(0)}%)</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_: unknown, r: MajorSummaryRow) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <div style={{ fontSize: 12, color: '#475569' }}>NN: <b>{r.publicArea}</b> · TN: <b>{r.privateArea}</b> · TT: <b>{r.selfEmployed}</b></div>
  ...
  const graduateColumns: ColumnsType<GraduateRow> = [
    {
      ...
      fixed: 'left',
      width: 200,
      render: (_: unknown, r: GraduateRow) => (
    ...
    },
    {
      ...
      width: 70,
      render: (v: string) => (
        <Tag color={v === 'Nam' ? 'blue' : 'pink'} style={{ borderRadius: 20, fontSize: 11, margin: 0 }}>{v}</Tag>
      ),
    },
    { title: 'Ngành', dataIndex: 'majorName', key: 'majorName', width: 160, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Khoa', dataIndex: 'facultyName', key: 'facultyName', width: 170, render: (v: string) => <span style={{ fontSize: 12, color: '#64748b' }}>{v}</span> },
    { title: 'QĐ tốt nghiệp', dataIndex: 'graduationDecision', key: 'graduationDecision', width: 130, render: (v: string) => <Text code style={{ fontSize: 12 }}>{v}</Text> },
    { title: 'Ngày ký', dataIndex: 'graduationDate', key: 'graduationDate', width: 100, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'SĐT', dataIndex: 'phone', key: 'phone', width: 120, render: (v: string) => <span style={{ fontSize: 12, color: '#475569' }}>{v}</span> },
    { title: 'Email', dataIndex: 'email', key: 'email', width: 170, render: (v: string) => <span style={{ fontSize: 12, color: '#475569' }}>{v}</span> },
    {
      title: 'H.thức KS',
      ...
      width: 120,
      render: (v: string) => {
        const colorMap: Record<string, string> = { Online: 'cyan', 'Điện thoại': 'blue', Email: 'geekblue', 'Phỏng vấn': 'purple' }
        return <Tag color={colorMap[v] || 'default'} style={{ borderRadius: 20, fontSize: 11, margin: 0 }}>{v}</Tag>
      },
    },
    {
      title: 'Phản hồi',
      ...
      render: (v: string) => (
        <Badge
          status={v === 'Đã phản hồi' ? 'success' : 'default'}
          text={<span style={{ fontSize: 12 }}>{v}</span>}
        />
      ),
    },
    { title: 'Ghi chú', dataIndex: 'note', key: 'note', width: 200, render: (v: string) => <span style={{ fontSize: 12, color: '#64748b' }}>{v}</span> },
  ]
  ...
  const responseColumns: ColumnsType<ResponseRow> = [
    {
      ...
      fixed: 'left',
      width: 190,
      render: (_: unknown, r: ResponseRow) => (
        <div>
          <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 13 }}>{r.fullName}</div>
          <div style={{ display: 'flex', gap: 4, marginTop: 2, alignItems: 'center' }}>
            <Text code style={{ fontSize: 11 }}>{r.studentCode}</Text>
            <Tag color={r.gender === 'Nam' ? 'blue' : 'pink'} style={{ borderRadius: 20, fontSize: 10, margin: 0, lineHeight: '16px', padding: '0 6px' }}>{r.gender}</Tag>
          </div>
        </div>
      ),
    },
    { title: 'Ngày sinh', dataIndex: 'birthDate', key: 'birthDate', width: 100, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Ngành', dataIndex: 'majorName', key: 'majorName', width: 160, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Khoa', dataIndex: 'facultyName', key: 'facultyName', width: 170, render: (v: string) => <span style={{ fontSize: 12, color: '#64748b' }}>{v}</span> },
    {
      title: 'Việc làm',
      ...
      render: (v: string) => {
        ...
        return (
          <span style={{ fontSize: 12, fontWeight: 500, color: s.color, background: s.bg, border: `1px solid ${s.border}`, borderRadius: 20, padding: '2px 10px', display: 'inline-block' }}>
            {v}
          </span>
        )
      },
    },
    { title: 'Khu vực', dataIndex: 'workArea', key: 'workArea', width: 130, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Tỉnh/TP', dataIndex: 'workProvince', key: 'workProvince', width: 100, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'TG tìm việc', dataIndex: 'jobFoundTime', key: 'jobFoundTime', width: 120, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Kiến thức', dataIndex: 'knowledgeLevel', key: 'knowledgeLevel', width: 120, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Lương KĐ', dataIndex: 'startingSalary', key: 'startingSalary', width: 110, render: (v: string) => <span style={{ fontSize: 12, fontWeight: 600, color: '#0f172a' }}>{v}</span> },
    { title: 'Thu nhập TB', dataIndex: 'averageIncome', key: 'averageIncome', width: 130, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Tìm việc qua', dataIndex: 'jobSearchMethod', key: 'jobSearchMethod', width: 150, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Tuyển dụng', dataIndex: 'recruitmentMethod', key: 'recruitmentMethod', width: 140, render: (v: string) => <span style={{ fontSize: 12 }}>{v}</span> },
    { title: 'Kỹ năng mềm', dataIndex: 'softSkill', key: 'softSkill', width: 190, render: (v: string) => <span style={{ fontSize: 12, color: '#64748b' }}>{v}</span> },
    { title: 'Khóa học thêm', dataIndex: 'extraCourse', key: 'extraCourse', width: 150, render: (v: string) => <span style={{ fontSize: 12, color: '#64748b' }}>{v}</span> },
    { title: 'Đề xuất', dataIndex: 'proposal', key: 'proposal', width: 200, render: (v: string) => <span style={{ fontSize: 12, color: '#64748b', fontStyle: 'italic' }}>{v}</span> },
  ]
  ...
  const exportMenu = {
    ...
    onClick: ({ key }: { key: string }) => {
      ...
      messageApi.success(`Đã xuất ${fmt} — phạm vi: ${scopeLabel(currentUser)}`)
    },
  }
  ...
  return (
    <AdminLayout>
      ...
      <style>{`
        ...
        .rp-table .ant-table-summary > tr > td {
        ...
        .rp-filter .ant-select-selector:hover {
        ...
        <div style={{
          ...
          <Space wrap size={8}>
            ...
            <Select
              ...
              placeholder="Chuyển role"
            />
            <Select
              value={filters.surveyId}
              onChange={v => set('surveyId', v)}
              options={surveys}
              style={{ width: 230 }}
              size="middle"
            />
            <Dropdown menu={exportMenu} placement="bottomRight">
              <Button icon={<DownloadOutlined />} type="default" style={{ borderRadius: 8, fontWeight: 500 }}>
                Xuất báo cáo
              </Button>
            </Dropdown>
          </Space>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {activeFilterCount > 1 && (
                <span style={{
                  ...
                  padding: '1px 8px', border: '1px solid #bfdbfe',
                }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Button
                ...
                icon={<ReloadOutlined />}
                onClick={e => { e.stopPropagation(); setFilters({ surveyId: '2026-1' }) }}
                style={{ color: '#94a3b8', fontSize: 12 }}
              >
                Đặt lại
              </Button>
              {filterOpen
                ? <CaretUpOutlined style={{ color: '#94a3b8', fontSize: 11 }} />
                : <CaretDownOutlined style={{ color: '#94a3b8', fontSize: 11 }} />
              }
            </div>
          </div>
                ...
                {[
                  {
                    ...
                    node: (
                      <Select
                        ...
                        allowClear
                        onChange={v => { set('facultyId', v); set('majorId', undefined) }}
                      />
                    ),
                  },
                  {
            ...
            items={[
              {
                ...
                label: (
                  <Space size={6}>
                    <BarChartOutlined />
                    Tổng hợp theo ngành
                    <span style={{ background: '#f1f5f9', color: '#475569', borderRadius: 20, fontSize: 11, padding: '0 7px', fontWeight: 600 }}>
                        ...
                        <Table.Summary fixed>
                          <Table.Summary.Row>
                            <Table.Summary.Cell index={0}>
                              <Text strong style={{ fontSize: 13 }}>TỔNG CỘNG</Text>
                            </Table.Summary.Cell>
                            <Table.Summary.Cell index={1}>
                              <Text type="secondary" style={{ fontSize: 12 }}>{scopeLabel(currentUser)}</Text>
                            </Table.Summary.Cell>
                            <Table.Summary.Cell index={2}>
                              <div style={{ fontSize: 13 }}>
                                <b>{stats.totalGraduates}</b>
                                <div style={{ fontSize: 11, color: '#94a3b8' }}>Phản hồi: {stats.totalResponses} ({stats.responseRate}%)</div>
                              </div>
                            </Table.Summary.Cell>
                            <Table.Summary.Cell index={3}>
                              <div style={{ fontSize: 13 }}>
                                <span style={{ color: '#15803d', fontWeight: 700 }}>{stats.employed}</span>
                                <span style={{ color: '#94a3b8', fontSize: 11 }}> ({stats.employmentRate}%)</span>
                              </div>
                            </Table.Summary.Cell>
                ...
                children: (
                  <div style={{ padding: '20px 0 24px' }}>
                          ...
                          <div key={row.key}>
                            ...
                            <div style={{ display: 'flex', gap: 16, marginTop: 5, fontSize: 11, color: '#64748b' }}>
                              ...
                              <span style={{ color: '#94a3b8', marginLeft: 'auto' }}>
                                Chưa PH: {row.totalGraduates - row.responses} ({(((row.totalGraduates - row.responses) / row.totalGraduates) * 100).toFixed(0)}%)
                              </span>
                            </div>
                          </div>
                        ))}
                    ...
                    <div style={{
                      ...
                      <BarChartOutlined style={{ color: '#94a3b8', fontSize: 16, marginTop: 2 }} />
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#475569' }}>Biểu đồ tương tác sẽ được bổ sung ở phase tiếp theo</div>
                        <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>
                          Reuse chart pattern từ màn Thống kê — cột phân phối, biểu đồ tròn khu vực, xu hướng theo đợt khảo sát.
                        </div>
                      </div>
                    </div>
                  </div>

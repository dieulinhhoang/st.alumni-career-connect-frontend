...
function ProgressBar({ value, total, color }: { value: number; total: number; color: string }) {
  ...
  return (
    <div style={{ width: 96 }}>
      ...
      <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 3, textAlign: "right" as const }}>
        {value}/{total} SV
      </div>
    </div>
  );
    ...
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px 18px 14px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 700, color: "#0f172a" }}>Tình trạng nộp báo cáo</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Select
            value={filter}
          ...
          <span style={{ fontSize: 12, color: "#64748b", background: "#f1f5f9", padding: "3px 11px", borderRadius: 99 }}>
            {latestDot}
          </span>
        </div>
      </div>
      ...
      <div style={{ maxHeight: 400, overflowY: "auto" }}>
        {filtered.map((k, i) => {
          ...
          return (
            ...
            >
              <span style={{ flex: 1, fontSize: 14, color: "#1e293b" }}>{k.ten}</span>
              <ProgressBar value={soSVPhanhoi} total={k.tongSV} color={k.mau} />
              <div style={{ minWidth: 95, textAlign: "right" as const }}>
                <span style={{
                ...
                {k.daNop && (
                  <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 3 }}>{k.ngayNop}</div>
                )}
              </div>
            </div>
          );

  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 12px 8px", flexShrink: 0, display: "flex", gap: 4, flexWrap: "nowrap", overflowX: "auto" }}>
        {categories.map((c) => (
          <button key={c} onClick={() => setCategory(c)} style={{ padding: "2px 8px", borderRadius: 20, border: "1px solid", borderColor: category === c ? accent : "#e8eaed", background: category === c ? `${accent}12` : "#fff", color: category === c ? accent : "#6b7280", fontSize: 12, fontWeight: category === c ? 700 : 400, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit", flexShrink: 0 }}>{c}</button>
        ))}
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "0 10px 12px", display: "flex", flexDirection: "column", gap: 5 }}>
        {filtered.map((bq) => (
          <div key={bq.id} draggable onDragStart={(e) => handleDragStart(e, bq)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", border: "1px solid #e8eaed", borderRadius: 8, cursor: "grab", background: "#fff", transition: "all .12s" }} onMouseEnter={(e) => { const d = e.currentTarget as HTMLDivElement; d.style.borderColor = accent; d.style.background = `${accent}08`; }} onMouseLeave={(e) => { const d = e.currentTarget as HTMLDivElement; d.style.borderColor = "#e8eaed"; d.style.background = "#fff"; }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 500, color: "#111827", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 2 }}>{bq.title}</div>
              <Space size={4}>
                <Tag style={{ fontSize: 11, lineHeight: "16px", padding: "0 5px", margin: 0 }}>{Q_TYPE_LABELS[bq.type] ?? bq.type}</Tag>
                <Tag color="cyan" style={{ fontSize: 11, lineHeight: "16px", padding: "0 5px", margin: 0 }}>{bq.category}</Tag>
              </Space>
            </div>
            <Tooltip title="Thêm vào form">
              <button
                onClick={() => { onAddFromBank(bq); onClose(); }}
                style={{ width: 26, height: 26, border: `1px solid ${accent}`, borderRadius: 6, background: `${accent}10`, color: accent, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0 }}
                aria-label="Thêm câu hỏi vào form"
              >
                <PlusOutlined />
              </button>
            </Tooltip>
        ...
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: "24px 12px", color: "#9ca3af", fontSize: 13 }}>Không tìm thấy</div>}
      </div>
    </div>
  );
}
        ...
        {THEME_PRESETS.map((theme) => {
          ...
          return (
            <div
              ...
              role="button"
              aria-label={`Chọn giao diện ${theme.name}`}
              aria-pressed={isSel}
              style={{
...
function LogicPanel({
  ...
  const removeRule = (id: string) => onRulesChange(rules.filter((rule) => rule.id !== id));
  ...
  if (questions.length < 2) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 8, color: "#9ca3af", padding: "32px 16px", textAlign: "center" }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: "#6b7280" }}>Cần ít nhất 2 câu hỏi</span>
        <span style={{ fontSize: 12, lineHeight: 1.6 }}>Thêm câu hỏi vào form trước khi thiết lập logic.</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px 8px", flexShrink: 0 }}>
        <span style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600 }}>{rules.length} điều kiện</span>
        <button
          onClick={addRule}
          aria-label="Thêm điều kiện logic"
          style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", border: `1.5px solid ${accent}`, borderRadius: 6, background: `${accent}10`, color: accent, cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit" }}
        >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
        {rules.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 120, gap: 6, color: "#c4c9d4", fontSize: 13, border: "2px dashed #e8eaed", borderRadius: 8, margin: "4px 0" }}>
          ...
          return (
            <div key={rule.id} style={{ border: "1px solid #e8eaed", borderLeft: `3px solid ${acColor}`, borderRadius: 8, padding: "10px 10px 8px", background: "#fafafa" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 800, color: acColor, background: `${acColor}12`, padding: "2px 7px", borderRadius: 10 }}>#{idx + 1} · {ACTION_LABELS[rule.action]}</span>
                <button
                  onClick={() => removeRule(rule.id)}
                  aria-label="Xóa điều kiện"
                  style={{ border: "none", background: "transparent", color: "#dc2626", cursor: "pointer", fontSize: 12, padding: 2, display: "flex", alignItems: "center", opacity: 0.7 }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 5, flexWrap: "wrap" }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: "#9ca3af", textTransform: "uppercase", minWidth: 20 }}>Nếu</span>
                ...
                <Select size="small" value={rule.operator} style={{ width: 96 }} onChange={(v) => updateRule(rule.id, { operator: v as LogicOperator })} options={Object.entries(OPERATOR_LABELS).map(([k, v]) => ({ value: k, label: v }))} />
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 5, paddingLeft: 24 }}>
                {hasOptions(rule.sourceQuestionId) ? (
                  ...
                  <input value={rule.value} onChange={(e) => updateRule(rule.id, { value: e.target.value })} placeholder="Giá trị..." style={{ flex: 1, height: 24, border: "1px solid #d9d9d9", borderRadius: 6, padding: "0 8px", fontSize: 12, outline: "none", fontFamily: "inherit" }} />
                )}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: "#9ca3af", textTransform: "uppercase", minWidth: 20 }}>Thì</span>
                <Select size="small" value={rule.action} style={{ width: 128 }} onChange={(v) => updateRule(rule.id, { action: v as LogicAction })} options={Object.entries(ACTION_LABELS).map(([k, v]) => ({ value: k, label: v }))} />
    ...
    return (
      <div style={{ width: "100%", height: "100%", background: "#fff", display: "flex", flexDirection: "column" }}>
        <div style={{ height: 44, display: "flex", alignItems: "center", padding: "0 12px", borderBottom: "1px solid #f0f2f5", gap: 4 }}>
          {PANEL_TABS.map((t) => (
            <button key={t.key} onClick={() => setActiveTab(t.key)} aria-label={t.label} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", border: "none", borderRadius: 6, background: activeTab === t.key ? `${accent}12` : "transparent", color: activeTab === t.key ? accent : "#9ca3af", cursor: "pointer", fontSize: 12, fontWeight: activeTab === t.key ? 700 : 400, fontFamily: "inherit" }}>{t.icon}</button>
          ))}
          <button onClick={onToggle} aria-label="Đóng panel" style={{ marginLeft: "auto", border: "none", background: "transparent", cursor: "pointer", color: "#9ca3af", fontSize: 16, display: "flex", alignItems: "center" }}><CloseOutlined /></button>
        </div>
        <div style={{ flex: 1, overflow: "hidden" }}>
          ...
          {activeTab === "logic" && <LogicPanel questions={questions} accent={accent} rules={logicRules} onRulesChange={handleRulesChange} />}
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", height: "100%", flexShrink: 0 }}>
      ...
      <div style={{ width: activeTab ? PANEL_WIDTH : 0, minWidth: 0, height: "100%", overflow: "hidden", transition: "width .22s cubic-bezier(.16,1,.3,1)", borderLeft: "1px solid #e8eaed", background: "#fff", display: "flex", flexDirection: "column" }}>
        <div style={{ height: 42, display: "flex", alignItems: "center", padding: "0 12px", borderBottom: "1px solid #f0f2f5", flexShrink: 0, background: "#fafafa", gap: 6, minWidth: PANEL_WIDTH }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: "#111827", flex: 1, whiteSpace: "nowrap" }}>{PANEL_TABS.find((t) => t.key === activeTab)?.label}</span>
          <button onClick={() => setActiveTab(null)} aria-label="Đóng panel" style={{ width: 24, height: 24, border: "1px solid #e8eaed", borderRadius: 6, background: "#fff", color: "#9ca3af", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, flexShrink: 0 }}><CloseOutlined /></button>
        </div>

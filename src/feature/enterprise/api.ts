...
// Sau nay se thay: api.get('/api/v1/enterprises'), api.get('/api/v1/jobs'), etc.

export const listEnterprises = async () => {
  const { data } = await api.get('/api/v1/enterprises');
  return data;
};

export const getEnterpriseById = async (id: string) => {
  const { data } = await api.get(`/api/v1/enterprises/${id}`);
  return data;
};

export const listJobs = async (enterpriseId?: string) => {
  const url = enterpriseId ? `/api/v1/enterprises/${enterpriseId}/jobs` : '/api/v1/jobs';
...
export const searchEnterprises = async (term: string) => {
  const { data } = await api.get('/api/v1/enterprises', { params: { q: term } });
  return data;
};

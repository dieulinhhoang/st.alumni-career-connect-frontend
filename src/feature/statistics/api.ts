...
export const getForms = async (): Promise<FormOption[]> => {
  const { data } = await api.get('/api/v1/statistics/forms');
  return data;
};

export const getStatisticalQuestions = async (formId: number): Promise<StatisticalQuestion[]> => {
  const { data } = await api.get(`/api/v1/statistics/questions/${formId}`);
  return data;
};
...
export const getFormStatisticsDetail = async (
  ...
  const { data } = await api.get(`/api/v1/statistics/detail/${formId}/${questionId}`);
  return data;
};
